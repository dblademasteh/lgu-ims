...
- **settings (sp-*)** — horizontal tab bar with accent underline, cards, toolbar search, tree/table toggle, toggle switches, status badges, API-key banner, QR/secret reveal.
  - **settings – Appearance tab** — first tab under Reference Data. Cards stack with `gap:1rem` via `.sp-animate-in { display:flex; flex-direction:column; gap:1rem; }`. Card shell `.sp-card` has no padding; header `.sp-card-head` carries `padding:1.5rem` and `border-bottom:1px solid var(--border)`; header-only cards remove the divider via `.sp-card-head:last-child { border-bottom:none; }`. Interactive content lives in `.sp-card-body { padding:1.25rem 1.5rem; gap:0.75rem; }`. Theme mode card is header-only with segmented control. Interface scale card shows range + labels in body. Font card uses `.sp-font-grid` (auto-fit minmax 9rem) with preview “Ag”. Accent card uses `.sp-swatch-grid` (≈7.5rem min) with preset swatches + custom color input. Preview card displays sample UI in `.sp-preview`.
...
- **CSV import modals** — Users, Suppliers, Items import flows use a consistent modal layout: `FormModal` with tone `info`, header with form number, `modal-body` for content, `modal-footer` for actions. Import modals feature a toolbar above the CSV fieldset with “Download template” and “Choose file” buttons (`btn btn-outline btn-sm gap-2`) plus a hint “or paste CSV below”. The fieldset uses `.fieldset` with `.fieldset-legend` and a mono `.textarea` for CSV paste. The form is wrapped in `.modal-body` to ensure standard `padding: 1.25rem 1.5rem` and proper spacing; footer actions remain in `.modal-footer`. Items import adds a “Template” button in the page header actions alongside “Import” to match the modal pattern.
- **Stock Adjustment modal** — official form `Stock Adjustment` / LGU-IMS-ADJ-01. Uses `.adj-modal` with accent bar and seal. Inputs with leading icons are wrapped in `.adj-input-wrap` with absolute-positioned `.adj-input-icon`. To avoid wrapper padding interfering with icon alignment, `.adj-modal .adj-input-wrap { padding:0; }` is enforced. Icons are placed at `left:0.75rem; top:50%; transform:translateY(-50%)` and inputs use `padding-left:2.5rem` / textarea `padding-left:2.5rem`. Wrapper padding must remain zero so the icon and text share the same baseline across light/dark themes.
